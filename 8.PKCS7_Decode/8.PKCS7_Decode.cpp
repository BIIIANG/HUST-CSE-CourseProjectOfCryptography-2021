...
int main(int argc, char** argv)
{
...
#ifdef _DEBUG
	freopen("..\\Examples\\第八题样例\\1.in", "r", stdin);
#endif // _DEBUG
	...
	for (int i = 0; i < signCount; i++)
	{
		...
		// set the cert to be verified
		X509_STORE_CTX* ctx = X509_STORE_CTX_new();
		X509_STORE_CTX_init(ctx, store, x509, NULL);

		// verify
		//ret = X509_verify_cert(ctx);
		//ret = PKCS7_verify(p7, p7->d.signed_and_enveloped->cert, store, NULL, NULL, 0);
		//int ret = PKCS7_signatureVerify(p7bio, p7, signInfo, getX509(certC));
